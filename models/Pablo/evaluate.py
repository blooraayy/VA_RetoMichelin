...
def load_gt_counts(coco_json: str) -> dict[str, int]:
    ...
    for ann in coco["annotations"]:
        ...
        # Roboflow adds suffix like ".rf.HASH" – strip it
...
def plot_summary_table(df: pd.DataFrame, outdir: str) -> None:
    ...
    fig.suptitle("Grounded-SAM — Summary metrics", fontsize=13, fontweight="bold", y=0.97)
    fig.tight_layout()
    _save(fig, outdir, "08_summary_table.png")
...
def print_aggregate_summary(df: pd.DataFrame, iou_thr: float) -> None:
    ...
    print()
    print(f"  -- Pixel-level segmentation --")
    ...
    print()
    print(f"  -- Detection-level  (IoU >= {iou_thr}) --")
...
def main() -> None:
    ...
    df = build_dataframe(summary, gt_counts, args.iou_thr)
