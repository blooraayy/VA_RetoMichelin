...
Pipeline:
  ...
  4. Guarda visualizaciones en outputs/figures/rubber_detection/
  5. Exporta bboxes detectados sobre imágenes raw a outputs/detections/rubber_detections.json

Uso:
  python rubber_detection.py                    # eval + visualizar + exportar JSON
  python rubber_detection.py --only-export      # solo exportar JSON (imágenes raw)
  python rubber_detection.py --only-eval        # solo métricas, sin guardar figuras
...
# ── Constantes ────────────────────────────────────────────────────────────────
RUBBER_CLASS  = 2
CONF_THR      = 0.65   # TPs tienen conf ~1.0; umbral alto evita FPs de fragmentos
IOU_THR       = 0.50
...
# ── Helpers de métricas ───────────────────────────────────────────────────────
def load_gt_rubber(label_path: Path, img_w: int, img_h: int) -> list:
    boxes = []
...
def save_figures_raw(model: YOLO) -> None:
    ...
    print(f"[viz-raw] Imágenes a procesar: {len(image_paths)}")
    ...
    print(f"[viz-raw] Figuras guardadas en: {OUT_FIG.resolve()}")
