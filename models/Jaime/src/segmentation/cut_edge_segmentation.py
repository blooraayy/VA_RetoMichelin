...
def segment_all(detections: dict, raw_dir: Path, sam, out_dir: Path):
    ...
    n = len(detections)
    print(f"[cut_edge_seg] Segmentando ({n} imágenes)...")
    t0 = time.time()
    ...
    for img_name, bboxes in sorted(detections.items()):
        ...
        ax.axis("off")
        fig.text(0.5, 0.01, f"{len(bboxes)} cut edge(s) segmentado(s)",
    ...
    print(f"[cut_edge_seg] Segmentación completada — {n} imágenes — {time.time()-t0:.1f} s")
...
if __name__ == "__main__":
    assert DETECTIONS_JSON.exists(), (
        ...
        "Ejecuta primero cut_edge_detection.py para generar el JSON."
    )
