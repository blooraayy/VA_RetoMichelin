...
The \textbf{MVTec Anomaly Detection (MVTec AD)} dataset was introduced in 2019 by 
Bergmann et al. at MVTec Software GmbH and is available at its official 
website\footnote{\url{https://www.mvtec.com/company/research/datasets/mvtec-ad}}. 
MVTec AD contains 5,354 high-resolution images divided into 15 categories, covering 
both textures (5 categories: carpet, grid, leather, tile, wood) and objects (10 
categories: bottle, cable, capsule, hazelnut, metal nut, pill, screw, toothbrush, 
transistor, zipper). The training set consists exclusively of defect-free images 
(3,629 images), while the test set includes both normal and anomalous samples (1,725 
images) with pixel-level ground truth annotations for 73 defect types. Images have 
resolutions between $700 \times 700$ and $1024 \times 1024$ pixels. MVTec AD is the 
primary benchmark for unsupervised anomaly detection and is used to evaluate 
AnomalyDINO \cite{Damm2024AnomalyDINO}, which achieves 96.6\% AUROC in the one-shot 
setting, and CLIP-DINOv2 \cite{Jiang2025CLIPDINO}, which achieves 93.4\% image-level 
AUROC in the zero-shot setting.\\
